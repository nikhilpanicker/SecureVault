...
class Writer :
	...
	def __init__(self) :
		self._critical_vector = []
		self._warning_vector = []
		self._notice_vector = []
		self._info_vector = []
	...
	def getCriticalVector(self):
		return self._critical_vector

	def getWarningVector(self):
		return self._warning_vector

	def getNoticeVector(self):
		return self._notice_vector
	...
	def startWriter(self, tag, level, summary, title_msg, special_tag=None, cve_number="") :	
		...
		if(level == LEVEL_CRITICAL) :
			self._critical_vector.append(tag)

		if(level == LEVEL_WARNING) :
			self._warning_vector.append(tag)

		if (level == LEVEL_NOTICE):
			self._notice_vector.append(tag)

		if(level ==  LEVEL_INFO) :
			self._info_vector.append(tag)
...
""", ["SSL_Security"])
	...
	writer.writeInf_ForceNoPrint("permission", all_permissions)
	writer.writeInf_ForceNoPrint("critical_vectors", writer.getCriticalVector() )
	writer.writeInf_ForceNoPrint("warning_vectors", writer.getWarningVector())
	writer.writeInf_ForceNoPrint("notice_vectors", writer.getNoticeVector() )
	writer.writeInf_ForceNoPrint("info_vectors", writer.getInfoVector())
